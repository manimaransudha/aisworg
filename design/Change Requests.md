...
A running register of change requests raised against the platform — feature additions, behavioural changes, and dev-tooling requests that fall outside the numbered MVP build phases. Newest first. Each entry records the request as raised, the agreed scope, the design decisions, and (once built) a Built banner.
