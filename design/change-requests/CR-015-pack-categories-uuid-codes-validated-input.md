...
> **§2 ("`code` is a system UUID") is DEPRECATED as of 2026-08-29 — do not implement it.** It held for about 5 days. CR-020 Part 2 (2026-08-19) replaced it: the interactive form's Code field became a required, Ontology-backed dropdown (`capability-name` concept type) — an author *picks* a real value, never a minted UUID. CR-020 itself flagged the gap that left open ("code's format is a UI convention, not a server-side constraint"); CR-046 closed it, adding real enforcement (`assertCanonicalCategory("capability-name", seed.code, …)` inside `validatePackSeed`) for every caller, not just the browser form. Confirmed live 2026-08-29: **zero Packs, any status including Draft, have ever had a UUID-shaped `code`** — every real Pack was seeded via the CLI/seed-file path with its own hand-typed code, and the interactive form's own required-field + Ontology validation blocks an empty submission before the old fallback could ever run. `toPackSeedInput`'s `randomUUID()` fallback (`core/sdkAuthoring.ts`) is dead code as of this date — left in place as a comment (not deleted) so a future reader sees why it's inert instead of rediscovering this from scratch. **Ch.5 §19.2/§19.11 still narrate the UUID design as current and need the same correction — not done here, flagged for a future pass.**
