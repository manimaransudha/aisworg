...
- **Objective *creation* authority.** Creation is not a governed transition, so it has no `noun_verb` and is **ungoverned** — it is now gated only by the gatekeeper (any authenticated user). The `requireRole('general')` that once fronted `/seu` was removed (role is landing-only, not authority), so creation carries **no** role or badge check at all. Closing it means either modelling creation as a birth transition (a `noun_verb` such as `objective_create`) or a dedicated create gate. (CR-009 adds *structural* create rules — mandatory parent, valid tier relationship — but these are integrity, not authority.)
...
**Broader principle (now largely realised for transitions):** *authority belongs on badges, not on `requireRole`.* CR-006 moved every governed transition onto `noun × verb`; what remains on `requireRole` is non-transition acts (creation, admin routes) — a separate, larger track.
