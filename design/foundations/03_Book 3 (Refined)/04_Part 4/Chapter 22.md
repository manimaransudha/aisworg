...
### 19.16 ✅ Thesis tested: does noun×verb generalise to every authority specification in this chapter? (2026-09-05)

Owner's thesis, tested directly against each ranked gap above: *"the nounxverb model should cater to any authority specification."* Walked one at a time, with two false starts corrected along the way before the pattern held:

- **§9 contextual authority by a fixed classification** (e.g. a security-critical Deliverable needing a stricter badge than a routine one) — **not a gap**: more states/nouns already express this (`Deliverable|SecurityCriticalUnderReview|SecurityCriticalApproved` requiring a different badge). No extension needed.
- **§9 contextual authority by *live* Obligation state** (a Deliverable's required badge tightening while a Critical Obligation is open against it) — **first false start**: initially raised as a gap noun×verb can't reach, since the fluctuating fact belongs to a different entity than the one transitioning. Corrected: this isn't Authority's job at all — Quality Gate already blocks the transition outright until Obligations clear (`no_unresolved_obligations`, real, exercised), and if an override is wanted, Quality Gate Waivers already exist for exactly that (`quality_gate_waivers`, badge-gated via `qualitygate_waive`, scoped to one blocked instance). Authority, Policy, and Quality Gate stay correctly separate, sequential checks (`transitionEngine.evaluate()`); nothing here needed folding into the badge model.
- **§8/§12 composition across tenants** (Cigna wants an additional badge on top of the Platform's, for its own tenant only) — closes by scoping the *row*: a nullable `tenant_id` on `transition_definitions`, `evaluate()` composing (AND) the platform-default row with a tenant-specific one when both exist. Still one badge lookup per applicable row.
- **§11 delegation** (time-boxed, traceable, auto-reverting) — closes by enriching the *grant*, not the check: `delegated_from`/`expires_at`/`scope` on `badge_grants`. `authorise()` still asks "does this actor hold a valid badge," now including time-boxed, provenance-tagged ones.
- **§10 non-binary outcomes** (Authorised with Conditions / Escalation Required / Waiver Required) — closes by widening the outcome enum and checking a small ranked set of badges instead of one (full badge → `Authorised`; conditional-tier badge only → `AuthorisedWithConditions`; neither → `NotAuthorised`).
- **§13 declared Separation of Duties** (not just an accident of who happens to hold which badges) — closes with one new declarative field on the transition row: `mustDifferFromActorOf: <a named prior transition on this same instance>`, checked against `events.actor_id` (already captured, 19.11) alongside the badge requirement.
- **§9 authority scoped to live per-SEU Capability fulfilment** (a badge holder with zero involvement in a given SEU shouldn't necessarily carry the same weight as the Participant actually fulfilling that SEU's relevant Capability) — closes by widening *where* "holds the badge" can be satisfied from: a static `badge_grants` row, or a live join against that SEU's own Capability Fulfilment record.
- **Stress test: multi-party/quorum approval** (two different departments must each independently sign off) — decomposes into more intermediate states (`UnderReview → PartiallyApproved → Approved`) plus the same differs-from-prior-actor constraint already proposed for Separation of Duties.

**Conclusion**: every gap in the ranked summary above closes with a bounded extension to one of three things — what counts as *holding* a badge (static grant, time-boxed delegation, or a live derived fact), what *scope* a required-badge row applies to (global vs. tenant, composed by AND), or the *outcome* shape (a small ranked enum, plus one same-instance actor-history constraint for Separation of Duties). None require a different paradigm (a general rules engine, or Pack-authored custom nouns/verbs) — the two-input badge lookup stays the primitive throughout.

**Decision: build none of it now.** Every extension above is a real, identified, bounded design — but speculative until a concrete situation actually needs one. Owner: *"No new implementation for authority rules unless we encounter a situation."* Authority Rules is accordingly the one Pack component type [CR-088](../../change-requests/CR-088-template-profile-configurable-parameters.md)'s own configurable-parameter walk closes with nothing to design at all — not "no configurable parameter identified" the way Review Gate/Quality Gate concluded, but "no new implementation, therefore nothing to make configurable, until one of the above is actually triggered by a real requirement."