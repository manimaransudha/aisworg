...
- **Phase 9 (Pack Platform maturity):** a test that a Pack published through the SDK (not a web form — SDK-001) is visible on the platform-wide Registry, and that its lifecycle transitions correctly over real HTTP.
...
## Status — Phase 9 (Pack Platform maturity) ✅ Done (2026-08-04)

Added one further test to `tests/web-flow.e2e.test.ts`, per this brief's own newly-added Phase 9 pointer above. Walked the flow by hand against the running app first, but with one real structural difference from every prior phase: Packs are **SDK-only** (Ch.39 SDK-001, "Every production Pack shall be created using the SDK") — there is no web form or API POST that creates a Pack, by design, so this test's fixture setup calls `publishPack` (the same function `pnpm pack:publish` calls) directly rather than pretending an HTTP creation path exists, then exercises the two routes that genuinely are HTTP-reachable:

- Registry: `GET /aisworg/seu/packs` → `200`, platform-wide, shows every published Pack Version (not one row per code — one row per immutable Version, newest first within each code).
- Pack transition: `POST /aisworg/seu/packs/:id/transition` (`targetState`) → `302` + flash, same shape as every other lifecycle transition endpoint in this app.

**One real test-writing wrinkle, not a bug**: after transitioning the fixture Pack, its code string appears *twice* on the re-fetched Registry page — once in the flash success message, once in the Pack's own card — and only the card's nearby `state-badge` reflects the new status (the flash message is just a sentence). A naive "find the pack code, look at what's near it" match caught the flash occurrence first and asserted against text that would never contain a state badge. Fixed by matching every occurrence and asserting against the *last* one (the actual card), not the first. Same category of thing this brief exists to catch — a plausible-looking assertion that silently checks the wrong part of the page — but caught here rather than shipped, since a raw substring match happened to still contain a state badge close enough to produce a *believable* wrong answer on the first attempt.

The test publishes and activates a real Pack via the SDK, confirms it shows on the Registry page with its correct code and version, transitions it to "Deprecated" over real HTTP with CSRF, and confirms the card (not just the flash message) now shows "Deprecated". Schema/dependency validation, Version immutability (republishing the same version is a no-op; a new version creates a new row and supersedes the previously-Active one), the Draft→Validated→Published→Active authority tiering (`general` vs `power`), and the two-Pack composition/override-conflict path are all covered in `tests/pack-sdk.test.ts` (direct-function layer) rather than duplicated at the HTTP layer.

Full suite: 67/67 passing (56 existing + 1 new HTTP-level test + 10 new direct-function tests in `tests/pack-sdk.test.ts`, plus one existing test in `tests/engine.test.ts` updated — not broken — to reflect the now-real 2-Pack composition the default Profile actually produces).
